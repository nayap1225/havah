...
!(function (a, b) {
        ...
        return (
          ...
          void 0 === c
            ...
            : ((f = typeof c),
              ...
              null != c &&
                c === c &&
                ('number' !== f || o.cssNumber[h] || (c += 'px'), l.clearCloneStyle || '' !== c || 0 !== b.indexOf('background') || (i[b] = 'inherit'), (g && 'set' in g && void 0 === (c = g.set(a, c, d))) || ((i[b] = ''), (i[b] = c))),
              void 0)
        );
      }
    },
  ...
  function Xb(a, b, c) {
    ...
    return (
      o.map(k, Ub, j), o.isFunction(j.opts.start) && j.opts.start.call(a, j), o.fx.timer(o.extend(i, { elem: a, anim: j, queue: j.opts.queue })), j.progress(j.opts.progress).done(j.opts.done, j.opts.complete).fail(j.opts.fail).always(j.opts.always)
    );
  }
              ...
              (b = function (a) {
                return function () {
                  b &&
                    ...
                    'abort' === a ? f.abort() : 'error' === a ? d(f.status, f.statusText) : d(Ec[f.status] || f.status, f.statusText, 'string' == typeof f.responseText ? { text: f.responseText } : void 0, f.getAllResponseHeaders()));
                };
              }),
                (f.onload = b()),
                (f.onerror = b('error')),
...
(function ($, window, document, undefined) {
  var feWorksheet = {
    ...
    setNavi: function (opt) {
      var _naviCode = '<nav>' + '<h2>' + opt.naviTitle + '</h2>' + '<ul></ul>' + '</nav>'; // navigation code
      ...
      return this.each(function () {
        ...
        // Creat nav items
        $navEl.find('ul').append('<li><a href="#' + opt.naviPrefix + thisIdx + '">' + $menuTitEl.text() + '</a></li>');
      });
    },
    ...
    setNumber: function (opt) {
      return this.each(function () {
        var $this = $(this), // workseetEl
          _theadCellCode = '<th scope="col" class="' + opt.numCellClass + '">' + opt.numCellText + '</th>'; // thead cell code
        // Create number cell
        $this
          .find('thead tr')
          ...
          .each(function () {
            ...
            var idxNum = $(this).index() + 1, // Table row index
              _tbodyCellCode = '<td class="' + opt.numCellClass + '">' + idxNum + '</td>'; // tbody cell code
    ...
    delayInfo: function (opt) {
      return this.each(function () {
        var $this = $(this), // dueCellClass
          ...
          tDate = new Date(),
          dFormat = /(\d{4})-(\d{2})-(\d{2})/,
          tDate = tDate.getFullYear() + '-' + (tDate.getMonth() + 1 < 10 ? '0' + (tDate.getMonth() + 1) : '' + (tDate.getMonth() + 1)) + '-' + (tDate.getDate() < 10 ? '0' + tDate.getDate() : '' + tDate.getDate());
        // Add class
        if (dDate == '' || !dFormat.test(dDate)) $this.addClass(opt.delayError); // Error
    ...
    _taskSetPage: function (opt) {
      return this.each(function () {
        ...
        $_taskCell.each(function () {
          // Add task data attr
          $(this).text().trim() == '' || $.inArray($(this).text(), taskKeyArray) == -1 ? $(this).attr('data-task', defaultTask).text(defaultTask) : $(this).attr('data-task', $(this).text());
        ...
        $.each(taskKeyArray, function (idx, val) {
          var $footerEl = $('footer'),
            taskSum = $_taskCell.filter('[data-task=' + val + ']').size(), // Get size
            _footerThead = '<th scope="col">' + '<i>' + taskNameArray[idx] + '</i>' + '<b>' + val + '</b>' + '</th>'; // footer thead code
          $footerEl
    ...
    _taskSetTable: function (opt, taskKeyArray, taskNameArray) {
      return this.each(function () {
        var $this = $(this), // workseetEl
          ...
          _totalCode = '<tr>' + '<th scope="row" colspan="' + colSpanSize + '">' + 'Total' + '</th>' + '<td>' + totalSize + '</td>' + '</tr>', // total code
          _footerTbodyCode = '<tr>' + '<th scope="row">' + menuTitle + '</th>' + '</tr>'; // tbody code in footer el
        $footerEl.find('tbody').append(_footerTbodyCode); // Add menu in footer tbody
        $this.append('<tfoot />'); // Create tfoot el
        $.each(taskKeyArray, function (idx, val) {
          var tgTaskSize = $taskCell.filter('[data-task=' + val + ']').size(), // Get task size
            _footerCode = '<td>' + tgTaskSize + '</td>', // tbody td code in footer el
            _tfootCode = '<tr>' + '<th scope="row" colspan="' + colSpanSize + '">' + '<i>' + taskNameArray[idx] + '</i>' + '<b>' + val + '</b>' + '</th>' + '<td>' + tgTaskSize + '</td>' + '</tr>'; // tfoot tr code
    ...
    _linkPath: function (opt) {
      return this.each(function () {
        var $this = $(this), // nameCellClass
          thisTxt = $this.text(),
          defaultPath = $this.closest(opt.workseetEl).find('caption').text().trim(), // caption el text
          folderPath = $this
        ...
        !folderPath ? (newPath = defaultPath + '/' + thisTxt) : (newPath = defaultPath + '/' + folderPath + '/' + thisTxt);
  ...
  $.fn.feWorksheet = function (method) {
    var arguments;
    feWorksheet[method] ? (arguments = feWorksheet[method].apply(this, Array.prototype.slice.call(arguments, 1))) : (arguments = feWorksheet.init.apply(this, arguments));
    return arguments;
  };
...
$(function () {
  $('.iframeName').each(function () {
    var link = $(this).text();
    var param = link.split('.')[0];
    console.log(param);
    $(this).html('<a href="../guide/guide.html?' + param + '" target="_blank">' + link + '</a>');
  });
});
